...
export class AcronymsService {
    ...
    this._http.get(url, { headers: headers }).subscribe((res: any) => {
      if (res) {
        ...
        if (user) {
          ...
          if (parsedUser['user_type'] === 'su' || parsedUser['user_type'] === 'admin') {
            this.suggestedAcronyms$.next(res);
            localStorage.setItem('suggested-acronyms', JSON.stringify(res));
          } else {
            const suggestedEmail = res.filter(
              (r: any) => r.suggested_by_email == parsedUser['email']
            );
  ...
  createSuggestedAcronym(suggestedAcronym: any) {
    ...
    if (user) {
      ...
      return this._http
        .post(url, suggestedAcronym, { headers: headers })
        .subscribe((data) => {
          this.messageService.add({
            severity: 'success',
            detail: `Acronym '${suggestedAcronym['acronym_name']}' is submitted and it's under review.`,
          });
  ...
  updateSuggestedAcronym(suggestedAcronym: any, editForm: any) {
    ...
    const user = localStorage.getItem('user');
    if (user) {
      if(JSON.parse(user)['user_type'] === 'user') {
        suggestedAcronym['suggested_by_name'] = JSON.parse(user)['user_name'];
        suggestedAcronym['suggested_by_email'] = JSON.parse(user)['email'];
      }
      suggestedAcronym['created_on'] = '';
      suggestedAcronym['status'] = suggestedAcronym['status'].code;
      if (suggestedAcronym['status'] === 'approved') {
        return this._http
          .post(`${environment.baseURL}/add`, suggestedAcronym)
          .subscribe((data) => {
            this.acronymUpdated$.next(true);
            this.messageService.add({
              severity: 'success',
              detail: `Acronym '${suggestedAcronym['acronym_name']}' is approved.`,
            });
      ...
      return this._http.put(url, suggestedAcronym).subscribe((data) => {
        ...
        this.getSuggestedAcronyms();
        this._http
          .get(`${environment.baseURL}/${suggestedAcronym['acronym_name']}`)
          .subscribe((data: any) => {
            if (data && data['acronym_name']) {
              this.deleteAcronym(suggestedAcronym['acronym_name']).subscribe();
            }
          });
        this.messageService.add({
          severity: 'success',
          detail: `Acronym '${suggestedAcronym['acronym_name']}' is updated.`,
        });
      });
    }
    return;
  ...
  updateAcronym(acronym: any) {
    ...
    if (user) {
      ...
      return this._http.put(url, acronym).subscribe((data) => {
        ...
        this.messageService.add({
          severity: 'success',
          detail: `Acronym '${acronym['acronym_name']}' is updated.`,
        });
        return this._http.put(`${environment.baseURL}/suggest/update`, acronym).subscribe(data => {
          localStorage.removeItem('suggested-acronyms');
          this.getSuggestedAcronyms();
        })
      });
  ...
  deleteSuggestedAcronym(acronym_name: string) {
    return this._http.delete(
      `${environment.baseURL}/suggest/delete/${acronym_name}`
    );
  }
